...
function auc = plotroc(y,x,show,params)
...
%   plotroc(y,x,model);
    ...
    elseif length(unique(y))~=2
       error('ROC is only applicable to binary classes with labels 1, -1'); % check the trainig_file is binary
        ...
        y_tmp=y;
        label=unique(y_tmp);
        y(find(y_tmp==label(1)))=-1;
        y(find(y_tmp==label(2)))=1;
        
        if exist('params') && ~ischar(params)

        model = params;

        [predict_label,mse,deci] = svmpredict(y,x,model) ;% the procedure for predicting
     
        auc = roc_curve(deci*model.Label(1),y,show);
        ...
        [param,fold] = proc_argv(params); % specify each parameter
           ...
           if fold <= 1
            ...
            auc = roc_curve(deci,label_y,show); % plot ROC curve
...
function auc = roc_curve(deci,label_y,show) %%deci=wx+b, label_y, true label
